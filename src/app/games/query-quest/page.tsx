          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400">
                Solv SQL queries against a real in-browser SQLite database &mdash; powered by sql.js (WASM).
              </p>
            </div>
          </div>
          <GameSocial slug={GAME_SLUG}  title="Query Quest" emoji={"\uD83D\uDDC3\uFE0F"} accentText="text-emerald-400" accentBg="bg-emerald-500/10" currentUser={currentUser} canInteract={gamesAuthed} onAuthRequired={() => { setAuthRequest("login"); setShowAuthModal(true); }} />
        </div>
          ...
          <section className="space-y-4">
            {booting && (
              <div className="flex items-center gap-3 rounded-2xl border border-emerald-500/30 bg-emerald-500/5 px-4 py-3 text-sm text-emerald-300">
                ...
                Downloading &amp; booting the SQLite engine (WASM ~640&nbsp;KB)&hellip;
              </div>
            )}
                ...
                <div className="mb-2 flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-semibold text-white">
                      Level {current.id} &middot; {current.title}
                    </h3>
                  </div>
                  {current.points > 0 && (
