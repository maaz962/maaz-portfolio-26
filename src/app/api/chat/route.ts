...
5) Animation Arena (/games/animation-arena) - 🤖 "Bring robots to life with CSS transitions, transforms & keyframes" -
12 levels in 2 tiers: beginner L1-L8 (8 XP each), intermediate L9-L12 (9 XP each). Max score 100 XP (8x8 + 4x9 = 100).
Level list: 1 First Fade (transition: background-color 0.4s - color fades in); 2 Easy Does It (transition: opacity 0.6s ease-in-out); 3 Slide Over (transform: translateX(80px) - a dashed goal ring shows the target spot); 4 Turn the Dial (transform: rotate(45deg)); 5 Super Size (transform: scale(1.5)); 6 First Loop (animation: pulse 1.5s - the game injects @keyframes pulse/spin/float/pop/wiggle); 7 Endless Motion (animation: spin 1s infinite - accepted as the shorthand OR as the longhand animation-name/animation-duration/animation-iteration-count trio); 8 On Repeat (animation: float 3s ease-in-out infinite - shorthand or longhand); 9 Late Arrival (animation: pop 2s ease-in-out AND animation-delay: 1s - it waits one second then pops); 10 Zoom In, Zoom Out (animation: pulse 2s ease-in-out infinite alternate - animation-direction makes it zigzag); 11 Hover Lift (transition: transform 0.3s ease AND transform: translateY(-8px) - the robot lifts on hover); 12 Grand Finale (transform: translateY(-8px) scale(1.1) - combines translate and scale; only transition/transform allowed).
Pass conditions: you type CSS property:value lines and the game checks them against accepted sets; multi-line levels need every listed property present; levels 7, 8 and 10 accept either the animation shorthand or the equivalent longhand properties; hover levels only apply your transform while the mouse is over the robot so you can watch it lift. Resets only clear your code - they never un-complete a solved level. Hints draw from a shared daily budget of 3 hints across ALL games, tracked server-side via /api/games/hints (every game shows "Hints left today: N" next to its Show Hint button).
Tier unlock: intermediate unlocks after all 8 beginner levels solved (arena levels unlock strictly in sequence).

6) PHP Playground (/games/php-playground) - 🐘 "Type real PHP and watch it run live in your browser" -
...
7) Query Quest (/games/query-quest) - 🗃️ "Master SQL by querying a real in-your-browser database" -
16 levels in 4 tiers: easy L1-L5 (5 XP each), intermediate L6-L10 (6 XP each), hard L11-L13 (7 XP each), mostHard L14-L16 (8 XP each). Max score 100 XP.
...
- XP is the score saved per game when you beat levels (XP per level differs per game as listed above). Total possible XP across all 7 games = 700 (100 HTM-100 GFP-100 FZX-100 JSD-100 AAA-100 PPP-100 QQ).
