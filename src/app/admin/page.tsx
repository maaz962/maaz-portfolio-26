...
import {
  ...
  Eye, Clock, MapPin, RefreshCw, ChevronDown, ChevronUp, Lock,
} from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/track", {
        headers: { "x-admin-secret": password || "maaz-analytics-2026" },
      });
      if (!res.ok) {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (password === "maaz-analytics-2026") {
  ...
  if (!authenticated) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-background p-4">
        <form onSubmit={handleLogin} className="w-full max-w-sm space-y-4 rounded-2xl border border-border bg-card p-6 shadow-card">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted">Enter password to view analytics</p>
            </div>
          </div>
          ...
          <button
            type="submit"
            className="w-full rounded-xl bg-primary px-4 py-2.5 text-sm font-medium text-primary-foreground hover:brightness-110 transition-all"
          >
            Access Dashboard
          </button>
        </form>
